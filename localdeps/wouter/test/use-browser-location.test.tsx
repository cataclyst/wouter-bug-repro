...
describe("`value` first argument", () => {
  ...
  test("back button navigation maintains correct render order", async () => {
    const renderOrder: string[] = [];

    const ComponentA = () => {
      useEffect(() => {
        renderOrder.push("A-render");
        return () => {
          renderOrder.push("A-unmount");
        };
      });
      return <div>A</div>;
    };
    ...
    const ComponentB = () => {
      useEffect(() => {
        renderOrder.push("B-render");
        return () => {
          renderOrder.push("B-unmount");
        };
      });
      return <div>B</div>;
    };
    ...
    const App = () => {
      ...
      return (
        <>
          {location === "/a" && <ComponentA />}
          {location === "/b" && <ComponentB />}
        </>
      );
    };

    // Start at /a
    act(() => history.replaceState(null, "", "/a"));
    const { unmount } = render(<App />);

    await waitFor(() => expect(renderOrder).toContain("A-render"));
    renderOrder.length = 0; // Clear for next test

    // Navigate to /b via pushState (like Link does)
    act(() => history.pushState(null, "", "/b"));
    await waitFor(() => expect(renderOrder).toContain("B-render"));

    // Expected order: A unmounts, then B renders
    expect(renderOrder[0]).toBe("A-unmount");
    expect(renderOrder[1]).toBe("B-render");
    renderOrder.length = 0; // Clear for back button test
    ...
    // Critical assertion: B should unmount BEFORE A renders
    // This was the bug - B was re-rendering after pressing back button
    expect(renderOrder[0]).toBe("B-unmount");
    expect(renderOrder[1]).toBe("A-render");

    // Ensure B didn't re-render (the bug we're fixing)
    expect(renderOrder.filter(x => x === "B-render")).toHaveLength(0);
